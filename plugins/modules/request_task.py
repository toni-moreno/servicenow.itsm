...
seealso:
  - module: servicenow.itsm.request_task_info
...
options:
  ...
  work_notes:
    description:
      - Notes visible by the workgroup members.
    type: str
  comments:
    description:
      - Comments visible by petitioner.
    type: str
  other:
    description:
...
- name: Close request task
  delegate_to: localhost
  servicenow.itsm.request_task:
    state: "Closed Complete"
    close_notes: "Closed Note {{ansible_facts.ansible_date_time.iso8601}}"
    work_notes: "Work Note {{ansible_facts.ansible_date_time.iso8601}}"
    comments: "Comments {{ansible_facts.ansible_date_time.iso8601}}"
    number: SCTASK0010001
  run_once: true
  register: result
"""
...
def ensure_absent(module, table_client, attachment_client):
    ...
    change = table_client.get_record("sc_task", query)
    ...
    if change:
        ...
        table_client.delete_record("sc_task", change, module.check_mode)
...
def ensure_present(module, table_client, attachment_client, variable_client):
    ...
    old = mapper.to_ansible(
        table_client.get_record("sc_task", query, must_exist=True)
    )

    old["attachments"] = attachment_client.list_records(
        dict(table_name="sc_task", table_sys_id=old["sys_id"])
    )
