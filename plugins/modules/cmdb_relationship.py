...
def create_relationship(client, all_relationships):
    ...
    return client.post('/api/now/cmdbrelation/add-rels',
                       payload, query=table._query()).json["result"]
...
def delete_relationship(client, all_relationships):
    ...
    return client.delete('/api/now/cmdbrelation/delete-rels',
                         query=q).json["result"]
...
def main():
    module_args = dict(
        ...
        relationship_type=dict(type='str', choices=[
                               "ci_downstream", "ci_upstream", ], required=True),
    )
